...
description: Build a project using Claude Code Agent Teams. Takes a plan document path and optional team size. Use when you want multiple agents collaborating on a build.
...
**Display mode** — choose based on your terminal:

- **In-process** (default, no setup): all teammates run in your main terminal. Use Shift+Up/Down to select a teammate. Works in any terminal.
- **Split panes**: each teammate gets its own pane. Requires tmux or iTerm2.

The default `teammateMode: "auto"` uses split panes if already in tmux, in-process otherwise. To force a mode, set in `settings.json`:
