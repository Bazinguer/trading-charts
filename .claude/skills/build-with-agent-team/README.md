...
- **In-process** (default): all teammates run inside your main terminal. Use Shift+Up/Down to select a teammate. **No extra setup required.**
- **Split panes**: each teammate gets its own pane for simultaneous visibility. Requires tmux or iTerm2.

The default is `"auto"` — uses split panes if already in tmux, in-process otherwise.

**To install tmux (only needed for split-pane mode):**
